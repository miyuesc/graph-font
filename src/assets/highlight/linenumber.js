export default function linenumber(hljs) {
  ...
  function addStyles() {
    ...
    css.type = "text/css";
    css.innerHTML = format(".{0}{border-collapse:collapse}" + ".{1}:before{content:attr({2})}", [TABLE_NAME, NUMBER_LINE_NAME, DATA_ATTR_NAME]);
      ...
      for (let i = 0, l = lines.length; i < l; i++) {
        html += format(
          "<tr>" +
            ...
            "</td>" +
            "</tr>",
          [NUMBERS_BLOCK_NAME, LINE_NAME, NUMBER_LINE_NAME, DATA_ATTR_NAME, CODE_BLOCK_NAME, i + 1, lines[i].length > 0 ? lines[i] : " "]
        );
      }
